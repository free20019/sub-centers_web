  ...
  <template>
    <t-query-panel :model="query" footer="300">
      ...
      <t-table-page :data="table.data" :loading="table.display" :page-size="20">
        ...
        <el-table-column prop="JOIN_TIME" label="转出时间" align="center" width="140"></el-table-column>
      ...
      <el-dialog :title="dialog.title" :visible.sync="dialog.display" width="400px" @closed="handleDialogClosed">
        <el-form ref="form" :model="dialog.form" size="small" label-width="100px" label-position="right">
          ...
          <el-form-item label="转出时间">
            <el-date-picker v-model="dialog.form.time" type="datetime" placeholder="转出时间" style="width: 260px;"></el-date-picker>
          </el-form-item>
        </el-form>
      ...
      methods: {
        ...
        handleExportClick() {
          ...
          this.$confirm('是否需要导出?', '提示', {
            ...
            window.open(`${baseURL}letterManagement/findTransferApplicationExcel?startTime=${startTime && moment(startTime).format('YYYY-MM-DD')}&endTime=${endTime && moment(endTime).format('YYYY-MM-DD')}&vehicle=${vehicle}&type=${1}`);
          }).catch(() => {
          });
        }
      }
