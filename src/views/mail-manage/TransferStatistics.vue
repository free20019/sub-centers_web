...
<template>
  <t-query-panel :model="query" footer="300">
    ...
    <t-table-page :data="table.data" :loading="table.display" :page-size="20">
      ...
      <el-table-column prop="COUNT" label="转入次数" align="center"  min-width="160" :resizable="false"></el-table-column>
    </t-table-page>
  </t-query-panel>
</template>
    ...
    methods: {
      ...
      handleExportClick() {
        ...
        this.$confirm('是否需要导出?', '提示', {
          ...
          window.open(`${baseURL}letterManagement/findTransferStatisticsExcel?startTime=${startTime && moment(startTime).format('YYYY-MM-DD HH:mm:ss')}&endTime=${endTime && moment(endTime).format('YYYY-MM-DD HH:mm:ss')}&vehicle=${vehicle}&company=${company}&status=0`);
        }).catch(() => {
        });
      }
    }
