...
<template>
  <t-query-panel :model="query" footer="300">
    ...
    <t-table-page :data="table.data" :loading="table.display" :page-size="20">
      ...
      <el-table-column prop="OWN_TEL" label="联系电话" align="center" min-width="240" :resizable="false"></el-table-column>
    </t-table-page>
  </t-query-panel>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      findUnoperatedVehicle() {
        ...
        axios.get('dataStatistics/findUnoperatedVehicle', {
          baseURL,
          params: {
      ...
      handleExportClick() {
        ...
        this.$confirm('是否需要导出?', '提示', {
          ...
          window.open(`${baseURL}dataStatistics/findUnoperatedVehicleExcel?startTime=${startTime && moment(startTime).format('YYYY-MM-DD')}&endTime=${endTime && moment(endTime).format('YYYY-MM-DD')}&company=${company}&vehicleGroupId=${vehicleGroupId}`);
        }).catch(() => {
