...
<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <t-query-panel :model="query" footer="300">
    <template v-slot:querybar>
      ...
      <el-form-item>
        <el-select
          v-model="query.vehicleGroupId"
          filterable
          clearable
          placeholder="请选择车辆组"
          v-if="loginType === '111111'"
        >
          <el-option
      ...
      <el-form-item>
        ...
        <el-button
          ...
          size="mini"
          disabled
          v-for="(item, index) in table.vehicle_mileage"
          :key="index"
          >{{ item + '公里' }}</el-button
        >
    ...
    <!-- <template v-if="table.dataDisabled === 1">
      <el-button
        ...
        size="mini"
        disabled
        v-for="item in table.vehicle_mileage"
        >{{ item + '公里' }}</el-button
      >
    </template> -->
...
export default {
  ...
  methods: {
    ...
    handleVehicleQuerySearch(query) {
      ...
      if (query.length >= 3) {
        ...
        setTimeout(() => {
          ...
          this.select.selectedVehicle = _.filter(
            this.select.vehicle,
    ...
    findCyclingMileageStatistics() {
      ...
      this.table.data = []
      axios
        .get('dataStatistics/findCyclingMileageStatistics', {
          baseURL,
          params: {
            startTime:
    ...
    handleExportClick() {
      ...
      this.$confirm('是否需要导出?', '提示', {
        ...
        .then(() => {
          window.open(
            `${baseURL}dataStatistics/findCyclingMileageStatisticsExcel?startTime=${startTime &&
              moment(startTime).format(
                'YYYY-MM-DD HH:mm:ss'
              )}&endTime=${endTime &&
              moment(endTime).format(
                'YYYY-MM-DD HH:mm:ss'
              )}&vehicle=${vehicle.toString()}&vehicleGroupId=${vehicleGroupId}`
          )
        })
        .catch(() => {})
    }
