...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.findNoOperatingVehicleOnLine();
      this.getCompany();
    },
    methods: {
      iconClassName,
      ...
      findNoOperatingVehicleOnLine() {
        ...
        axios.get('dataStatistics/findNoOperatingVehicleOnLine', {
          baseURL,
          params: {
      ...
      handleExportClick() {
        ...
        this.$confirm('是否需要导出?', '提示', {
          ...
          window.open(`${baseURL}dataStatistics/findNoOperatingVehicleOnLineExcel?startTime=${startTime && moment(startTime).format('YYYY-MM-DD')}&endTime=${endTime && moment(endTime).format('YYYY-MM-DD')}&company=${company}&vehicleGroupId=${vehicleGroupId}`);
        }).catch(() => {
